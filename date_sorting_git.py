...
csv_file = '/Users/finleydavis/Desktop/csvs final/carrizo_wilcox_final.csv'
# variavble data now has my aquifer csv file loaded into it
data = pd.read_csv(csv_file)

# this is the method to sort my data by date
data['Year'] = data.iloc[:, 4].apply(lambda x: x[-4:] if isinstance(x, str) and x[-4:].isdigit() else None)
...
data = data.dropna(subset=['Year', 'Well Depth (ft. below land surface)'])
...
data.iloc[:, 4] = data['Year']

# drops the year column if you don't want to keep it in the final output
data = data.drop(columns=['Year'])

# save the sorted CSV to a new file 
data.to_csv('/Users/finleydavis/Desktop/csvs final/carrizo_wilcox_final_filtered.csv', index=False)
