...
def bootstrap_theil_sen(x, y, n_bootstrap=1000, ci=0.10):
    ...
    for _ in range(n_bootstrap):
        idx = np.random.choice(len(x), len(x), replace=True)  # Resample with replacement
        sample_x, sample_y = x[idx], y[idx]
        slope, _, _, _ = theilslopes(sample_y, sample_x)
        slopes.append(slope)
    
    # Compute 10% confidence interval percentiles
    lower_bound = np.percentile(slopes, (50 - ci * 50))  # 45th percentile
    upper_bound = np.percentile(slopes, (50 + ci * 50))  # 55th percentile
...
def analyze_aquifer_data(file_path, aquifer_name, start_year=1920, end_year=2020, output_folder=None):
    ...
    # Calculate annual lognormal means
    ...
    try:
        # Run change point detection only if there are enough data points
...
analyze_aquifer_data(file_path = aquifers['Gulf Coast']['path'], aquifer_name = 'Gulf Coast', start_year=1920, end_year=2020,)